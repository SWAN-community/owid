...
|-------------------------|-------------|--------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| /owid/api/v3/creator    | Mandatory   | date (optional): unsigned 32 bit integer, minutes since 01/01/2020 UTC, the OWID's own Date value (see Data Structure) | Provides a JSON response with the OWID creator information. The fields returned must include the domain, common name and public key that is also available via the public-key end point. The following is an example JSON response for the domain [new-pork-limes.uk](https://new-pork-limes.uk/owid/api/v1/creator). { "domain": "new-pork-limes.uk", "name": "New Pork Limes", "publicKeySPKI": "…" } Other fields could be included in time such as the [DUNS number](https://www.dnb.co.uk/duns-number/lookup.html), [registered company number](https://find-and-update.company-information.service.gov.uk/), or the URL of the applicable privacy policy. Creators that rotate signing keys MAY accept the optional date parameter here as on the public-key end point; the same rules apply (support is OPTIONAL, the key with the latest creation date on or before date is returned, and a date before the oldest known key returns 404), with the selected key reported in the publicKeySPKI field. |
| /owid/api/v3/public-key | Mandatory   | format: valid values are “spki” or “pkcs”. date (optional): unsigned 32 bit integer, minutes since 01/01/2020 UTC — the OWID's own Date value (see Data Structure)                                                                                      | Returns the public key associated with the creator in Subject Public Key Info (SPKI) or Public Key Cryptography Standards (PKCS) form. The response is a text string in ECDSA Public Key form. For example: -----BEGIN PUBLIC KEY----- MFwwDQYJKo… /zrPytDbp\\naa == -----END PUBLIC KEY----- Creators that rotate signing keys MAY accept the optional date parameter; support is OPTIONAL and a creator MUST NOT be considered non-conformant for omitting it. When date is supplied to a creator that supports it, the creator MUST return the key whose creation date is the latest on or before date, and MUST return a 404 response when date predates the oldest known key. Clients MUST be prepared for a creator that ignores date and returns the current key; they SHOULD cache the public key locally and send date only as a fallback after a cached key fails to verify an OWID.                                                                                                                                                                                                       |
